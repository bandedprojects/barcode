        ...
        if (batch) {
            if (found) {
                ...
                this.batchService.rejectedCylindersList(data).subscribe(responseData => {
                    ...
                    this.barcode_weight = parseFloat(this.printBarcodeForm.value.weight).toFixed(1);
...
class BatchService {
    ...
    prepareTareWeigthReportDataSource(start, end, tareweightCylinders) {
        ...
        for (let i = 0; i < col_length; i++) {
            ...
            for (var j = 1; j <= 2; j++) {
                ...
                batchItem = Object.assign({}, batchItem, { [serial_no_index]: start_slno, [tare_weight_left]: tare_weight_left_value, [tare_weight_right]: tare_weight_right_value, [tareweight]: tareweight_value });
            }
            this.batchData.push(batchItem);
            ++start_row_sl_no;
        }
    ...
    prepareDispatchDataSource(data) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            batchItem = Object.assign({}, batchItem, { [serial_no_index]: data[i].serial_number, [status_key]: status });
