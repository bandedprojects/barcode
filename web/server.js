...
app.post('/tareweightcylinders',  (req, res) => {
  ...
  dbconnctor.executeQuery('SELECT DISTINCT * FROM tare_weight_info WHERE batchname="'+cylinder.batch_name+'" and batchtype="'+cylinder.batchtype+'" ORDER BY serial_number ASC', (err, data)=>{
    console.log(data);
    if (data.length != 0) {
